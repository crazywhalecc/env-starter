...
on_list_input_enter_space() {
  ...
  tput cuf $((${#prompt}+3))
  printf "${theme_color}${_list_options[$_list_selected_index]}${normal}"
  tput el
...
_list_input() {
  ...
  print "${normal}${theme_color}?${normal} ${bold}${prompt}${normal} ${dim}(使用方向键选择)${normal}"

  for i in $(gen_index ${#_list_options[@]}); do
    tput cub "$(tput cols)"
    if [ $i = 0 ]; then
      print "${theme_color}${arrow} ${_list_options[$i]} ${normal}"
    else
